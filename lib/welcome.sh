...
arr=("Hello genius! Today you are the best coder - Shengfan Zhou" 
       ...
       "Prototype to learn. Prototyping is a learning experience. It's value lies not in the code you produce but in the lessons you learn." 
...
one_line=""
arr[index]=${arr[index]%' '}
max_one_line_count=8
for str in ${arr[index]}; do

    b=$(( count % ${max_one_line_count} ))
    if [ $b = 0 ]; then
        startify="$startify $str\n"
        
        one_line="$one_line $str"
        if (( ${#one_line} > ${max_line_count} )); then
            max_line_count=${#one_line}
        fi
        one_line=""
    else
        one_line="$one_line $str"
...
if (( ${count} < ${max_one_line_count} )); then
    max_line_count=${#one_line}
fi

upper=${edge[0]}
for ((count=1;count<${max_line_count};count++)); do
    upper="${edge[1]}$upper"
done
echo
echo "${edge[0]}$upper"
